...
#define NUMBER_FOLDING_FACTOR 5
...
void formatMatrixTable (){
	int i=0, j=0, a=0;

	for(a=0; a<matrixSize;a++){
		strcpy(matrix[a].dest, "");
		strcpy(matrix[a].dest_port, "");
		strcpy(matrix[a].source, "");
		strcpy(matrix[a].source_port, "");

		for(i=0; i<13; i++){
			for (j=0; j<12; j++){
				matrix[a].table[i][j]=0;
			}
		}
	}
...
struct Matrix createTable (int startPoint, int endPoint, int startPoint2, int endPoint2){
		...
		for(j=0; j<dataParseLog[i].numberTag; j++){
			...
			if(matrixID !=-1){
				...
				if(newTag == 1 || newTag ==2){//new session "hello_request" "client_hello"
					...
					matrix[matrixID].table[0][newTag] ++;
	...
	for(i =startPoint2; i < endPoint2; i++){
			int j=0;
			for(j=0; j<dataParseLog[i].numberTag; j++){
				int newTag = dataParseLog[i].tag[j];
...
float distanceCalculator (struct Matrix alfa, struct Matrix beta){
	int i,j;
	double distance =0;

	for(i =0; i<15; i++){
		for(j =0; j<15; j++){
			distance = distance + (alfa.table[i][j]-beta.table[i][j]) *(alfa.table[i][j]-beta.table[i][j]);
		}
	}
...
int main(int argc, char *argv[]){
    ...
    int totalElementPerChute = dataParseLogSize/NUMBER_FOLDING_FACTOR;
    printf("\n=>Total data parsed: #%d There are %d \n", dataParseLogSize, totalElementPerChute);

    double avgDistance = 0;
    int z=0, startPoint = 0, endPoint = totalElementPerChute;

	struct Matrix test  = createTable(0, totalElementPerChute,0,0);
	struct Matrix validation  = createTable(totalElementPerChute, dataParseLogSize,0,0);
	double distance= distanceCalculator(test, validation);
	avgDistance = avgDistance + distance;
	printf("The distance in round %d is %f\n", 1, distance);

	test  = createTable(totalElementPerChute, totalElementPerChute+totalElementPerChute,0,0);
	validation  = createTable(0, totalElementPerChute,totalElementPerChute+totalElementPerChute,dataParseLogSize);
	distance= distanceCalculator(test, validation);
	avgDistance = avgDistance + distance;
	printf("The distance in round %d is %f\n", 2, distance);

	test  = createTable(totalElementPerChute+totalElementPerChute, totalElementPerChute+totalElementPerChute+totalElementPerChute,0,0);
	validation  = createTable(0, totalElementPerChute+totalElementPerChute,totalElementPerChute+totalElementPerChute+totalElementPerChute,dataParseLogSize);
	distance= distanceCalculator(test, validation);
	avgDistance = avgDistance + distance;
	printf("The distance in round %d is %f\n", 3, distance);

	test  = createTable(totalElementPerChute+totalElementPerChute+totalElementPerChute, totalElementPerChute+totalElementPerChute+totalElementPerChute+totalElementPerChute,0,0);
	validation  = createTable(0, totalElementPerChute+totalElementPerChute+totalElementPerChute,totalElementPerChute+totalElementPerChute+totalElementPerChute+totalElementPerChute,dataParseLogSize);
	distance= distanceCalculator(test, validation);
	avgDistance = avgDistance + distance;
	printf("The distance in round %d is %f\n", 4, distance);

	test  = createTable(totalElementPerChute+totalElementPerChute+totalElementPerChute+totalElementPerChute, dataParseLogSize,0,0);
	validation  = createTable(0, dataParseLogSize-totalElementPerChute,0,0);
	distance= distanceCalculator(test, validation);
	avgDistance = avgDistance + distance;
	printf("The distance in round %d is %f\n", 5, distance);
    ...
    //Round two is the smallest one
    printf("These two table are similar\n");
	 ...
	 fOutGraph = fopen (OUTPUT_GRAPH_FILE,"w");
        ...
        for(x=0; x<15; x++){
            for(y=0; y<15; y++){
            	printf("%.0f ", test.table[x][y]*100);
             	fprintf(fOutTable, "%.0f ", test.table[x][y]*100);

            	if(test.table[x][y]!=0){
                	fprintf(fOutGraph, " %s -> %s[label=\"%.0f\",weight=\"%.0f\"];\n", database2[x], database2[y], test.table[x][y]*100, test.table[x][y]*100);
            	}
            }
            printf("\n");
         	fprintf(fOutTable, "\n");
		...
		for(x=0; x<15; x++){
			for(y=0; y<15; y++){
				printf("%.0f ",validation.table[x][y]*100);
				fprintf(fOutTable, "%.0f ", validation.table[x][y]*100);

				if(validation.table[x][y]!=0){
					fprintf(fOutGraph, " %s -> %s[label=\"%.0f\",weight=\"%.0f\"];\n", database2[x], database2[y], validation.table[x][y]*100, validation.table[x][y]*100);
				}
			}
			printf("\n");
			fprintf(fOutTable, "\n");
